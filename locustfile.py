...
ORDER_URL = "http://ec2-35-178-165-226.eu-west-2.compute.amazonaws.com:8080"
PAYMENT_URL = "http://ec2-35-178-165-226.eu-west-2.compute.amazonaws.com:8080"
STOCK_URL = "http://ec2-35-178-33-177.eu-west-2.compute.amazonaws.com:8080"
USER_URL = "http://ec2-3-8-121-205.eu-west-2.compute.amazonaws.com:8080"

jsonHeader = {'content-type': 'application/json'}

def create_item(self):
    price = random.randint(1, 10)
    response = self.client.post(f"{STOCK_URL}/stock/item/create/{price}", json={}, name="/stock/item/create/[price]")
...
def add_stock(self, item_idx: int):
    ...
    self.client.post(f"{STOCK_URL}/stock/add/{self.item_ids[item_idx]}/{stock_to_add}",
                     json={}, name="/stock/add/[item_id]/[number]")


def create_user(self):
    response = self.client.post(f"{USER_URL}/users/create", json={}, name="/users/create/")
...
def add_balance_to_user(self):
    ...
    self.client.post(f"{USER_URL}/users/credit/add/{self.user_id}/{balance_to_add}",
        json={}, name="/users/credit/add/[user_id]/[amount]")


def create_order(self):
    response = self.client.post(f"{ORDER_URL}/orders/create/{self.user_id}",
        json={}, name="/orders/create/[user_id]")
...
def add_item_to_order(self, item_idx: int):
    response = self.client.post(f"{ORDER_URL}/orders/addItem/{self.order_id}/{self.item_ids[item_idx]}",
        json={}, name="/orders/addItem/[order_id]/[item_id]", catch_response=True)
